...
int main() {
  ...
  while (t--) {
    ...
    cin >> n >> q;
    vector<int> a(n);
    vector<int> pref(n, 0); // prefix sum
    for (int i = 0; i < n; i++) {
      cin >> a[i];
      pref[i] = a[i] + (i == 0 ? 0 : pref[i - 1]);
      assert(a[i] >= 1 && a[i] <= MAX_N);
    }
    int l, r, k;
    // if we change all elements in range l, r to k
    // will the sum be odd?
    for (int i = 0; i < q; i++) { // queries
      ...
      if (pref[n - 1] % 2) {
        // original sum odd
        odd =
            abs(k * (r - l + 1) - (pref[r - 1] - (l == 1 ? 0 : pref[l - 2]))) %
                2 ==
            0;
      } else {
        // original sum even
